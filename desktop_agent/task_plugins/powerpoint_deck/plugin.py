...
_WORK_TERMS = ("插件", "aoryn", "演示", "汇报", "展示", "文稿", "presentation", "deck", "demo")

_SLIDES: list[tuple[str, list[str]]] = [
    ("Aoryn 桌面智能代理系统", ["目标：让代理观察屏幕、判断下一步、操作软件。", "运行重点：稳定任务、插件扩展、可追溯证据。"]),
    ("核心闭环", ["截图与视觉理解", "大模型/规则脑进行决策", "鼠标键盘和软件插件执行动作"]),
    ("稳定任务", ["计算器完成 1+1", "浏览器阅读多个网页并写 Typora 报告", "画图工具逐笔绘制图形", "MATLAB 插件生成函数曲线"]),
    ("插件机制", ["每个插件声明触发词、能力、状态和默认任务。", "前端自动展示插件目录。", "任务路由可以把专业软件任务交给对应插件。"]),
    ("后续扩展", ["为 QQ、MATLAB、Office、CAD 等软件添加专用插件。", "把通用视觉代理与软件专用 API/脚本结合。", "提升运行稳定性，同时保留可解释的执行证据。"]),
]
...
def run_task(
    ...
    *,
    ...
    actions = office_common.emit(context, "PowerPoint 插件正在生成 Aoryn 汇报文稿")
    ...
    if executable is not None:
        try:
            ...
            pptx_error = str(exc)
    if not pptx_created:
        office_common.write_basic_pptx(pptx_path, "Aoryn PowerPoint 插件文稿", _SLIDES)
        pptx_created = True
        mode = "标准 PPTX 文件生成" if pptx_error is None else "标准 PPTX 文件生成（COM 不可用时降级）"
    office_common.write_html_deck(html_path, "Aoryn PowerPoint 插件文稿", _SLIDES)
    ...
    answer = (
        "✅ PowerPoint 插件任务已完成：已生成 Aoryn 汇报文稿和插件报告。\n\n"
        f"执行方式：{mode}\n"
        f"PPTX：{pptx_path}\n"
        f"HTML：{html_path}\n"
        f"报告：{report_path}"
    ...
    return PluginRunResult(
        completed=True,
        headline="PowerPoint 插件已完成：生成 Aoryn 汇报文稿",
        answer=answer,
        actions=actions,
        artifacts=artifacts,
    )
