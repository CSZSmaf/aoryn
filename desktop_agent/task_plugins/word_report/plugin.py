...
_SECTIONS: list[tuple[str, list[str]]] = [
    ("插件架构", ["插件通过 plugin.json 声明身份、触发词、能力和演示任务。", "后端自动发现插件并通过 plugin:ID 路由任务。", "前端从 task_plugins 元数据渲染插件能力卡片。"]),
    ("当前插件", ["MATLAB 绘图插件：调用 MATLAB 批处理生成曲线图。", "Excel 报表插件：生成工作簿、趋势图和分析报告。", "PowerPoint 演示插件：生成五分钟汇报演示稿。", "Word 报告插件：生成 DOCX 汇报文档。"]),
    ("演示价值", ["可说明系统不是只靠固定脚本，而是具备按软件扩展的接口。", "插件能把专业软件的稳定 API/COM 能力和通用视觉代理结合。", "每个插件都会留下产物和运行记录，方便现场追溯。"]),
    ("后续计划", ["为 QQ 群聊、CAD、MATLAB 更多函数、浏览器站点定制插件。", "补充插件安装、启停和权限配置界面。", "增加插件市场式目录和插件运行日志。"]),
]
...
def run_task(
    ...
    *,
    ...
    actions = office_common.emit(context, "Word 插件正在生成 DOCX 插件能力报告")

    office_common.write_basic_docx(docx_path, "Aoryn 插件能力报告", _SECTIONS)
    ...
    answer = (
        "✅ Word 插件任务已完成：已生成 DOCX 插件能力报告和 Markdown 说明。\n\n"
        f"DOCX：{docx_path}\n"
        f"报告：{report_path}"
    )
    return PluginRunResult(
        completed=True,
        headline="Word 插件已完成：生成 Aoryn 插件能力报告",
        answer=answer,
        actions=actions,
