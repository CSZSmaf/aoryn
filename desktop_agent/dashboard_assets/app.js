...
function applyStaticCopy() {
  ...
  if (helpTitle) {
    helpTitle.textContent = tr("帮助与高级", "Help and advanced");
  }
  const helpHint = document.getElementById("helpAndAdvancedHint");
  if (helpHint) {
    helpHint.textContent = tr(
      "普通对话可以回答产品问题，高级入口里保留开发控制台。",
      "Chat mode answers product questions. The developer console lives in the advanced entry."
    );
  }
  const helpCenterTitle = document.getElementById("helpCenterTitle");
  if (helpCenterTitle) {
    helpCenterTitle.textContent = tr("帮助中心", "Help center");
  }
  const helpCenterSubtitle = document.getElementById("helpCenterSubtitle");
  if (helpCenterSubtitle) {
    helpCenterSubtitle.textContent = tr("这里展示与 README 同步的使用说明。", "This view mirrors the README guidance.");
  }
  elements.helpBackdrop?.setAttribute("aria-label", tr("关闭帮助中心", "Close help center"));
  elements.closeHelpButton?.setAttribute("aria-label", closeLabel);
  if (elements.openAboutButton) {
    elements.openAboutButton.textContent = state.locale === "en-US" ? "About & Logs" : "关于与日志";
  }
  if (elements.aboutAndLogsHint) {
    elements.aboutAndLogsHint.textContent =
      state.locale === "en-US"
        ? "Check the app version, open config and log folders, and jump back to recent runs."
        : "查看版本信息、打开配置与日志目录，以及快速回看最近运行记录。";
  }
  const aboutAndLogsTitle = document.getElementById("aboutAndLogsTitle");
  if (aboutAndLogsTitle) {
    aboutAndLogsTitle.textContent = state.locale === "en-US" ? "About & Logs" : "关于与日志";
  }
  if (elements.aboutTitle) {
    elements.aboutTitle.textContent = "About Aoryn";
  }
  if (elements.aboutSubtitle) {
    elements.aboutSubtitle.textContent = state.locale === "en-US" ? "Version, diagnostics, and logs" : "版本、诊断与运行记录";
  }
  elements.aboutBackdrop?.setAttribute("aria-label", state.locale === "en-US" ? "Close about panel" : "关闭关于面板");
...
function renderSettingsProfile() {
  elements.configBadge.textContent = state.meta?.config_path
    ? `${tr("配置：", "Config: ")}${state.meta.config_path}`
    : tr("配置：默认", "Config: defaults");
}
...
function renderSettingsProfile() {
  ...
  if (profileTitle) {
    profileTitle.textContent = tr("Aoryn 工作台", "Aoryn Workspace");
  }
...
function renderOnboardingGuide() {
  ...
  const providerValue = elements.modelProvider?.value || state.meta?.defaults?.model_provider || "";
  const modelValue = elements.modelName?.value?.trim() || state.meta?.defaults?.model_name || "";
  const providerReady = Boolean(providerValue);
  const modelReady = Boolean(modelValue || elements.modelAutoDiscover?.checked);
  const browserReady = Boolean(elements.browserDomBackend?.value || state.meta?.defaults?.browser_dom_backend);
  const diagnostics = state.meta?.diagnostics || {};
  const checks = [
    {
      label: tr("选择模型提供方", "Choose a model provider"),
      done: providerReady,
      detail: providerReady ? providerValue : tr("尚未选择。", "Not selected yet."),
    },
    {
      label: tr("确认模型或自动发现", "Confirm a model or enable auto discover"),
      done: modelReady,
      detail: modelReady ? (modelValue || tr("自动发现已开启", "Auto discover enabled")) : tr("尚未完成。", "Not configured yet."),
    },
    {
      label: tr("确认浏览器执行方式", "Confirm the browser execution mode"),
      done: browserReady,
      detail: browserReady
        ? tr("DOM 与 GUI 设置已就绪。", "DOM and browser settings are ready.")
        : tr("浏览器设置还没检查。", "Browser settings still need a quick check."),
    },
  ];
  ...
  elements.onboardingSection.innerHTML = `
    <div class="onboarding-card">
      <div class="onboarding-card__head">
        <div>
          <p class="onboarding-card__eyebrow">${escapeHtml(tr("首次启动引导", "First launch guide"))}</p>
          <h3 class="onboarding-card__title">${escapeHtml(tr("先把基础环境配好，再开始第一条任务。", "Set up the basics before you run your first task."))}</h3>
          <p class="onboarding-card__body">${escapeHtml(tr("这里会自动保存你的连接、模型和浏览器设置。完成一次后，下次就不会再自动弹出。", "Aoryn will remember your connection, model, and browser settings. Once you finish this guide, it will not auto-open again."))}</p>
        </div>
        <span class="onboarding-card__badge">${escapeHtml(tr("首次启动", "First run"))}</span>
      </div>
      ...
      <div class="onboarding-actions">
        <button class="secondary-button" type="button" data-onboarding-provider="lmstudio_local">${escapeHtml(tr("使用本地 LM Studio", "Use local LM Studio"))}</button>
        <button class="secondary-button" type="button" data-onboarding-provider="openai_compatible">${escapeHtml(tr("使用兼容 API", "Use hosted / compatible API"))}</button>
        <button class="secondary-button" type="button" data-open-about="true">${escapeHtml(tr("查看版本与日志", "Open about & logs"))}</button>
      </div>
      <div class="onboarding-actions onboarding-actions--compact">
        <button class="secondary-button" type="button" data-onboarding-later="true">${escapeHtml(tr("稍后再说", "Maybe later"))}</button>
        <button class="primary-button" type="button" data-onboarding-complete="true">${escapeHtml(tr("完成引导", "Mark setup done"))}</button>
      </div>
      <p class="onboarding-card__footer">${escapeHtml(tr(`数据会保存在 ${diagnostics.data_dir || "AppData"}，运行记录会保存在 ${diagnostics.run_root || "runs"}。`, `Data will be stored in ${diagnostics.data_dir || "AppData"}, and run history will be stored in ${diagnostics.run_root || "runs"}.`))}</p>
    </div>
  `;
}
...
function renderAboutPanel() {
  ...
  const runtimeMode = state.meta?.runtime_mode === "packaged" ? tr("已安装应用", "Installed app") : tr("源码运行", "Source runtime");
  const pathRows = [
    { key: "config_dir", label: tr("配置目录", "Config folder"), value: diagnostics.config_dir || "-" },
    { key: "data_dir", label: tr("数据目录", "Data folder"), value: diagnostics.data_dir || "-" },
    { key: "run_root", label: tr("运行记录", "Run logs"), value: diagnostics.run_root || "-" },
    { key: "cache_dir", label: tr("缓存目录", "Cache folder"), value: diagnostics.cache_dir || "-" },
    { key: "install_dir", label: tr("安装目录", "Install folder"), value: diagnostics.install_dir || "-" },
  ];

  elements.aboutContent.innerHTML = `
    <div class="about-grid">
      <section class="about-card">
        <p class="about-card__eyebrow">${escapeHtml(tr("关于", "About"))}</p>
        <h3>${escapeHtml(appTitle)}</h3>
        <div class="about-metrics">
          <article class="about-metric"><span>${escapeHtml(tr("版本", "Version"))}</span><strong>v${escapeHtml(version)}</strong></article>
          <article class="about-metric"><span>${escapeHtml(tr("运行方式", "Runtime"))}</span><strong>${escapeHtml(runtimeMode)}</strong></article>
        </div>
        <p class="about-card__copy">${escapeHtml(tr("桌面执行工作台，支持普通对话、Agent 任务、运行记录和桌面壳悬浮执行态。", "A desktop workspace for chat, agent execution, run history, and the floating execution shell."))}</p>
        <div class="button-row">
          <button class="secondary-button" type="button" data-copy-diagnostics="true">${escapeHtml(tr("复制诊断信息", "Copy diagnostics"))}</button>
        </div>
      </section>

      <section class="about-card">
        <p class="about-card__eyebrow">${escapeHtml(tr("路径", "Paths"))}</p>
            ...
            .map(
              (item) => `
                <div class="about-path-row">
                  ...
                  <button class="secondary-button" type="button" data-open-path-key="${escapeHtml(item.key)}">${escapeHtml(tr("打开", "Open"))}</button>
                </div>
              `
            )
            .join("")}
      ...
      <section class="about-card about-card--wide">
        <p class="about-card__eyebrow">${escapeHtml(tr("日志与运行记录", "Logs & run history"))}</p>
        ${
          recentRuns.length
                  ...
                  .map(
                    (run) => `
                      <div class="about-run-row">
                        ...
                        <button class="secondary-button" type="button" data-open-run-id="${escapeHtml(run.id)}">${escapeHtml(tr("查看", "View"))}</button>
                      </div>
                    `
                  )
                  .join("")}
              </div>`
            : `<div class="empty-state">${escapeHtml(tr("还没有运行记录。", "No runs yet."))}</div>`
        }
      </section>
    </div>
  `;
...
function renderHelpCenter() {
  ...
  elements.helpContent.innerHTML = `<article class="help-doc help-doc--markdown assistant-markdown">${renderChatMarkdown(state.helpContent)}</article>`;
}
...
function renderNormalChat() {
  ...
  const draft = session && state.chatStreamDraft?.sessionId === session.id ? state.chatStreamDraft : null;
  const messages = [];
  const sessionMessages = session?.messages || [];
  const latestCompletedAssistantMessageId = draft ? null : getLatestCompletedAssistantMessage(sessionMessages)?.id || null;

  if (!sessionMessages.length && !draft) {
    messages.push(`
      <div class="chat-welcome">
    ...
    for (const message of sessionMessages) {
      if (message.role === "user") {
        ...
        messages.push(
          renderNormalAssistantMessage(message, {
            showActions: message.id === latestCompletedAssistantMessageId,
          })
        );
      }
    }
    if (draft) {
      messages.push(renderNormalAssistantPendingMessage(draft));
    }
  }
...
async function submitChatMessage(text) {
  ...
  try {
    await streamChatReply(requestBody, {
      onStart: () => {
        state.chatStreamDraft = {
      ...
      onDelta: (payload) => {
        ...
        if (!delta) return;
        if (!state.chatStreamDraft || state.chatStreamDraft.sessionId !== activeSession.id) {
          state.chatStreamDraft = {
            sessionId: activeSession.id,
            content: "",
            waiting: false,
        ...
        state.chatStreamDraft.waiting = false;
        state.chatStreamDraft.content += delta;
        scheduleChatStreamRender();
      },
      onDone: (payload) => {
        clearChatStreamRenderTimer();
        state.chatPending = false;
        const finalContent = String(payload?.assistant_message || state.chatStreamDraft?.content || "").trim();
    ...
    appendChatMessage(activeSession.id, {
      role: "assistant",
      content:
        error?.message || tr("普通对话请求被模型拒绝或执行失败，请检查模型配置和 provider 返回。", "The chat request was rejected or failed. Check the model and provider response."),
    });
    renderAll();
  }
}
...
async function loadHelpContent() {
  ...
  const payload = await fetchJson("/api/help");
  state.helpLoading = false;
  if (!payload) {
    state.helpError = tr("无法加载帮助中心。", "Could not load the help center.");
    renderAll();
    return;
  }
...
function renderHelpCenter() {
  ...
  if (state.helpLoading) {
    elements.helpContent.innerHTML = `<div class="empty-state">${escapeHtml(tr("正在加载帮助内容…", "Loading help content..."))}</div>`;
    return;
  }
  ...
  if (!state.helpContent) {
    elements.helpContent.innerHTML = `<div class="empty-state">${escapeHtml(tr("这里会显示产品帮助文档。", "Help content will appear here."))}</div>`;
    return;
  }
...
async function submitChatMessage(text) {
  ...
  await requestChatReply({
    sessionId: activeSession.id,
    requestMessages: buildChatRequestMessages(activeSession.messages || []),
  });
}
...
function renderOnboardingGuide() {
  ...
  const checks = [
    {
      label: isEnglish ? "Choose a model provider" : "选择模型提供方",
      done: providerReady,
      detail: providerReady ? providerValue : (isEnglish ? "Not selected yet." : "尚未选择。"),
    },
    {
      label: isEnglish ? "Confirm a model or enable auto discover" : "确认模型，或开启自动发现",
      done: modelReady,
      detail: modelReady
        ? (modelValue || (isEnglish ? "Auto discover enabled" : "已开启自动发现"))
        : (isEnglish ? "Not configured yet." : "尚未完成配置。"),
    },
    {
      label: isEnglish ? "Confirm the browser execution mode" : "确认浏览器执行方式",
      done: browserReady,
      detail: browserReady
        ? (isEnglish ? "DOM and browser settings are ready." : "DOM 与浏览器设置已经就绪。")
        : (isEnglish ? "Browser settings still need a quick check." : "浏览器设置还需要快速检查。"),
    },
  ];

  const footerText = isEnglish
    ? `Data will be stored in ${diagnostics.data_dir || "AppData"}, and run history will be stored in ${diagnostics.run_root || "runs"}.`
    : `数据会保存在 ${diagnostics.data_dir || "AppData"}，运行记录会保存在 ${diagnostics.run_root || "runs"}。`;
    ...
    <div class="onboarding-card">
      <div class="onboarding-card__head">
        <div>
          ...
          <h3 class="onboarding-card__title">${escapeHtml(isEnglish ? "Set up the basics before you run your first task." : "先把基础环境配置好，再开始第一条任务。")}</h3>
          <p class="onboarding-card__body">${escapeHtml(isEnglish ? "Aoryn will remember your connection, model, and browser settings. Once you finish this guide, it will not auto-open again." : "Aoryn 会记住你的连接、模型和浏览器设置。完成一次后，下次就不会再自动弹出。")}</p>
        </div>
      ...
      <div class="onboarding-actions">
        ...
        <button class="secondary-button" type="button" data-open-about="true">${escapeHtml(isEnglish ? "Open about & logs" : "查看版本与日志")}</button>
      </div>
      <div class="onboarding-actions onboarding-actions--compact">
      ...
      <p class="onboarding-card__footer">${escapeHtml(footerText)}</p>
    </div>
  `;
}
...
function renderAboutPanel() {
  ...
  const appTitle = state.meta?.title || "Aoryn";
  const version = state.meta?.version || APP_VERSION;
  const runtimeMode = state.meta?.runtime_mode === "packaged"
  ...
  if (elements.aboutSubtitle) {
    elements.aboutSubtitle.textContent = isEnglish ? "Version, diagnostics, and logs" : "版本、诊断与运行记录";
  }
  ...
  const pathRows = [
    ...
    { key: "run_root", label: isEnglish ? "Run logs" : "运行记录", value: diagnostics.run_root || "-" },
  ...
  elements.aboutContent.innerHTML = `
    <div class="about-grid">
      <section class="about-card">
        ...
        <p class="about-card__copy">${escapeHtml(isEnglish ? "A desktop workspace for chat, agent execution, run history, and the floating execution shell." : "一个面向桌面执行的工作台，支持普通对话、Agent 任务、运行记录以及悬浮执行态。")}</p>
        <div class="button-row">
...
function renderOnboardingGuide() {
  ...
  const checks = [
    ...
    },
    {
      ...
      detail: browserReady
        ? isEnglish
          ? "DOM and browser settings are ready."
          : "DOM 与浏览器设置已经就绪。"
        : isEnglish
          ? "Browser settings still need a quick check."
          : "浏览器设置还需要快速检查。",
    },
  ];

  const footerText = isEnglish
    ? `You can install Aoryn to a custom folder such as D:\\Apps\\Aoryn, but your config still stays in ${diagnostics.config_dir || "%APPDATA%\\\\Aoryn"} and run history still stays in ${diagnostics.run_root || "%LOCALAPPDATA%\\\\Aoryn\\\\runs"}.`
    : `Aoryn 可以安装到自定义目录（例如 D:\\Apps\\Aoryn），但配置仍保存在 ${diagnostics.config_dir || "%APPDATA%\\\\Aoryn"}，运行记录仍保存在 ${diagnostics.run_root || "%LOCALAPPDATA%\\\\Aoryn\\\\runs"}。`;
...
function renderAboutPanel() {
  ...
  const dataPolicyText = isEnglish
    ? "The install folder can be customized, but Aoryn always keeps config in %APPDATA%\\Aoryn and logs, cache, screenshots, and run history in %LOCALAPPDATA%\\Aoryn. During uninstall, you can choose whether to remove that user data."
    : "安装目录可以自定义，但 Aoryn 会固定把配置保存在 %APPDATA%\\Aoryn，把日志、缓存、截图和运行记录保存在 %LOCALAPPDATA%\\Aoryn。卸载时你可以选择是否一并删除这些用户数据。";
  ...
  elements.aboutContent.innerHTML = `
    <div class="about-grid">
      ...
      <section class="about-card">
        <p class="about-card__eyebrow">${escapeHtml(isEnglish ? "Paths" : "路径")}</p>
        <p class="about-card__copy">${escapeHtml(dataPolicyText)}</p>
        <div class="about-paths">
          ${pathRows
            .map(
              (item) => `
...
function applySupplementalStaticCopy() {
  ...
  if (elements.aboutAndLogsHint) {
    elements.aboutAndLogsHint.textContent =
      state.locale === "en-US"
        ? "Check the app version, confirm the custom install folder, open config and data folders, and jump back to recent runs."
        : "查看版本、确认自定义安装目录，并快速打开配置目录、数据目录和最近的运行记录。";
  }
  const aboutAndLogsTitle = document.getElementById("aboutAndLogsTitle");
  if (aboutAndLogsTitle) {
    aboutAndLogsTitle.textContent = state.locale === "en-US" ? "About & Logs" : "关于与日志";
